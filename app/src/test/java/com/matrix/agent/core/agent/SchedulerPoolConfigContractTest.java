...
/**
 * V0.5.3 评审 P1-4:schedulerPool 与 ioPool 拒绝策略配置契约。
 *
 * <p>V0.5.3 之前 AppContainer.schedulerPool 用 3 参默认 CallerRunsPolicy,
 * 与 V0.5.2-rev 已经改成 AbortPolicy 的 ioPool 不一致——队列满时 caller 线程同步执行,
 * 让 TaskScheduler.submit catch 成死代码。
 *
 * <p>本测试验证 V0.5.3 生产配置契约:DynamicThreadPool 4 参重载 + AbortPolicy 的 handler
...
public final class SchedulerPoolConfigContractTest {
    ...
    /**
     * V0.5.3 P1-4:schedulerPool 生产配置(core=2 / max=2 / queue=32 / AbortPolicy)的 handler
     * 必须是 AbortPolicy,与 ioPool(V0.5.2-rev P1-1)一致。
     */
    @Test
    ...
    /**
     * V0.5.3 P1-4 契约:schedulerPool 与 ioPool 用同一类 handler(AbortPolicy)。
