...
/**
 * V0.5.5 P1-A:即便 Provider 自己没有 handler-level gate,PolicyEngine 仍守住
 * memory.semantic.save 的显式许可约束。
 *
 * <p>用户原话:"保留 Provider 内的检查作为 defence-in-depth,但不能把它当唯一 gate"。
 * V0.5.4 把 gate 放在 MockCapabilityProvider.MemorySemanticSaveHandler,未来接入真实 AAOS
 * Provider 或第三方 CapabilityProvider(可能漏写 handler gate)就会绕过用户许可规则。
 * V0.5.5 把 gate 上移到 PolicyEngine 后,任何 Provider 替换都受同一约束。
 *
...
public final class ProviderSwapStillEnforcedTest {
     ...
     * 这类 Provider 会直接被调到——本测试验证 V0.5.5 后这不可能发生。
     */
