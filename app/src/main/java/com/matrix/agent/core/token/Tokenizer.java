...
/**
 * V0.5.0 Stage 5:Tokenizer 契约——AgentBudget token 维度估算入口。
 *
 * <p>V0.5.0 主路径(AgentEngine.estimateConversationChars / enforceMessageBudget)仍用 char,
 * 本接口仅提供能力。V0.5.1 切换判断时:
 * <ol>
 ...
 * </ol>
 *
 * <p>V0.5.0 默认装配 {@link CharFallbackTokenizer}(纯 text.length),保证向后兼容。
 */
public interface Tokenizer {
     ...
     * V0.5.0 用于诊断 tokenizer 异常(V0.5.1 切换主路径前的 sanity check)。
     */
    String encodeAndBack(String text);

    /**
     * V0.5.2 Stage 1:实现是否成功初始化(可走精确 BPE,而非退 char fallback)。
     *
     * <p>默认 true(CharFallbackTokenizer / 未来 fake stub 都算可用);jtokkit 实现可能 false
     * (EncodingException 注册失败 / 词表缺失)。仅诊断信号——主路径失败时仍走 fallback,
     * 不阻塞 Loop;Stage 3 接入 AgentEngine 主路径时 R3 缓解依赖此信号监控 jtokkit 健康度。
     */
