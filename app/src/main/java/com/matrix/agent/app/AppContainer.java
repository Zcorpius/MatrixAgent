...
public final class AppContainer {
    ...
    /**
     * Room/SQLCipher Audit 落库入口——注入 AgentEngine。
     *
     * <p>装配失败(初始化异常 / SQLCipher 不可用)时退化为 {@link NoopAuditRepository},
     * 保证启动不被 Audit 阻塞;fail-open 与 persist fail-open 一致。
     */
    private final AuditRepository auditRepository;
    /**
     * 四层 Memory 召回器——buildSystemPrompt + LlmPlanner.savedKeysFor 用。
     *
     * <p>包装 4 个 source:Working(SessionContext)/ Episodic(空)/ Semantic(空)/
     * Preference(SharedPreferencesMemoryStore)。
     */
    private final MemoryRecaller memoryRecaller;
    /**
     * jtokkit O200K_BASE Tokenizer——接入 AgentEngine 主路径。
     *
     * <p>仅装配骨架,AgentEngine 主路径仍走 char-based(零行为变更);
     * 双轨并行(同时记 char/token),切主路径(token 决策)。
     */
    ...
    /**
     * MemoryStore 装配降级标志——database=null 或 RoomMemoryStore 构造抛时
     * set true,UI 显示"记忆已降级"banner。volatile 保证 UI 线程读到最新值。
     *
     * <p>旧路径(此前)静默退到 SharedPreferencesMemoryStore(明文 XML),违反
     * "不静默降级"硬约束;改用 InMemoryMemoryStore(非持久化)+ 显式标志。
     */
    private volatile boolean memoryDegraded;
    /**
     * 端侧模型下载：Application Context（ModelDownloadViewModel 用作 cacheFile 根目录、
    ...
    public AppContainer(Context context) {
        ...
        //
        // schedulerPool 也改 AbortPolicy(与 ioPool 一致)。
        ...
        // 是死代码,TaskState.REJECTED / StopReason.REJECTED 永远走不到。
        ...
        // ioPool 改 AbortPolicy——队列满时显式抛 RejectedExecutionException,
        ...
        // 车辆运动状态源——保留为字段,通过 getVehicleStateSource()
        ...
        // 端侧模型下载：从 database 取 ModelDownloadDao + 构造 ModelDownloadManager。
        ...
        this.modelDownloadManager = downloadDao != null
                ...
                : null;
        // 启动期同步 DAO 与文件系统——修正 kill/重装/外部清理导致的状态漂移
        ...
        // createMemoryStoreSafely 3 参重载,失败时 set memoryDegraded=true
        ...
        // Memory 装配——Working / Episodic / Semantic / Preference 4 层 Router。
        // 提前到 modelGatewayRepository 之前:LlmPlanner.savedKeysFor 需要它。
        // Episodic / Semantic 升级真实 Room-backed 召回源——
        // database=null 时退 EmptyXxx(与 auditRepository fail-open 语义对齐)。
        // 保留 EpisodicMemorySourceImpl 引用,供 RoomMemoryWriter 调
        ...
        // MemoryWriter 装配提前到 provider 之前——
        ...
        // 启动链路只解密一次——classifier(L238)与 gateway(L346)复用同一 savedConfig，
        ...
        // 5 参构造器——把 memoryRecaller 透传给 LlmPlanner。
        ...
        // 装配 FallbackIntentClassifier(LlmIntentClassifier, Keyword)。
        ...
        // LLM 用旧 config 失败时 Fallback 自动退 Keyword,语义安全(后续增强)。
        IntentClassifier appClassifier = KeywordIntentClassifier.INSTANCE;
        ModelConfig savedForClassifier = savedConfig; // 复用启动期缓存的 savedConfig
        if (savedForClassifier != null && savedForClassifier.protocol == ApiProtocol.ON_DEVICE) {
            // 端侧离线，启动即走 Keyword，不建 LlmIntentClassifier（避免打云端）
        ...
        // SQLCipher + Room Audit 装配——fail-open,失败退化 NoopAuditRepository。
        // database 由 AppContainer 顶层装配,与 MemoryStore 共用同一实例。
        // 把此 auditRepository 注入 AgentEngine 5 个出口点。
        ...
        // 13 参构造器——传入 auditRepository + memoryRecaller。
        AgentRuntimeRepository.AgentEngineFactory engineFactory = gateway -> {
            ...
            // 装配期注入 digest(AgentEngine 默认构造时已退化为 SHA-1,
            // 这里覆盖为 HMAC-SHA-256 / 或装配失败时仍为 SHA-1)。
            engine.setAuditDigest(auditDigest);
            // 注入 Tokenizer——主路径双轨统计用,不影响 budget 决策。
            // 切换主路径到 token 时,AgentBudget 改用真实 jtokkit 估算。
            engine.setTokenizer(tokenizer);
            // 注入 AuditEventRecorder——5 个出口点写入 audit_event 表。
            engine.setAuditEventRecorder(auditEventRecorder);
            // 注入 PromptBuilder——buildSystemPrompt 委托 segmented 拼装,
            // 字面与内联路径等价(PromptBuilderEquivalenceTest 验证),PromptComposer
            // 在此基础上加 token 截断。
            engine.setPromptBuilder(new DefaultPromptBuilder());
            // 注入 ConversationCompressor——heuristic 降级(provider=null),
            // 长任务 BUDGET_EXHAUSTED 前先丢老 turns 腾预算。LLM 摘要路径留待后续
            // 装 LlmSummaryProvider——基于 Provider 的对话摘要,
            ...
            // 端侧 gateway → null SummaryProvider（ConversationCompressor heuristic 降级，离线不打云端）
            ...
            // 注入 MemoryWriter——主路径 L678 + terminalOutcome L783 写入
        ...
        agentRuntimeRepository = new AgentRuntimeRepository(engineFactory, provider, sessionManager,
                ...
                auditRepository);
        // Repository 清理用户数据时同步清空 SteerMailbox(陈旧 FORCE_TOOL/REPROMPT/DEFER)。
        // mailbox 与 AgentEngine 共享同一实例,不重复创建。
        agentRuntimeRepository.setSteerMailbox(steerMailbox);
        // 注入端侧 gateway 生命周期管理器——切模型时异步退役旧端侧 gateway（lease 等 JNI 返回后 close）
        ...
        // 注入增量 audit_event recorder,clearUserDataDetailed 时
        ...
        // 用户硬约束——"长期记忆仅由用户决定"。装配 KeywordMemoryIntentDetector,
        ...
        ModelConfig saved = savedConfig; // 复用启动期缓存的 savedConfig
        if (saved != null) {
    ...
    /**
     * 统一关闭——Repository(取消在途)→ schedulerPool → ioPool → auditEventRecorder。
     *
    ...
    /**
     * MatrixDatabase 装配——audit + memory 共用单一 SQLCipher 实例。
     *
     ...
     * 这种退化不阻塞 AppContainer 启动,与既有 fail-open 语义一致。
     */
    ...
    /**
     * Room + SQLCipher Audit 装配——fail-open。
     *
     * <p>database 由 {@link #createDatabaseSafely} 装配(database=null 时退 Noop)。
    ...
    private static AuditRepository createAuditRepositorySafely(MatrixDatabase database) {
        ...
        try {
            // 5 参构造器——持有 database + 4 DAO(加 auditEventDao),
    ...
    /**
     * audit_event 增量事件 Recorder 装配——fail-open。
     *
     ...
     * 现有范围内 sessionId 只有 "demo-driver" / "demo-passenger" 两个(ActorUsers
     * 集中定义),hard-code 不丢扩展性;接入真实多 session 时,改为运行时注入。
     */
    ...
    /**
     * 装配审计侧 HMAC-SHA-256 digest,失败 fail-closed。
     *
     * <p><b>fail-closed vs master_key fail-closed / vs 初版 fail-degrade</b>:
     * <ul>
     ...
     *       但初版退回 {@link Sha1AuditDigest}——这反而重暴露了要修的
     *       "SHA-1 8 位对低熵文本可枚举比对"风险。改 fail-closed:
